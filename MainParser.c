...
extern Cast_void startParser(Cast_void);
extern int startScanner(BufferPointer sc_buf);

static Cast_void printParserError(str fmt, ...);
static Cast_void displayParser(BufferPointer ptrBuffer);
static Cast_long getParserFileSize(str fname);
static Cast_void callGarbageCollector(Cast_void);
...
Cast_void printParserError(str fmt, ...) {
...
Cast_long getParserFileSize(str fname) {
	FILE* input;
	Cast_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
Cast_void displayParser(BufferPointer ptrBuffer) {
...
Cast_void callGarbageCollector(Cast_void) {
